...
- Двусторонняя GitHub-синхронизация DNS одной кнопкой с выбором более новой версии по времени до секунды.
...
### Автоматическая синхронизация DNS с GitHub
...
напрямую в интерфейсе Keenetic, кнопка `Считать текущую версию` перечитывает их и присваивает снимку текущую секунду.

Кнопка `Обновить` сравнивает версии и содержимое:

- более новая версия роутера записывается в GitHub через Contents API;
- более новая версия GitHub скачивается и автоматически строит/обновляет DNS-группы на роутере;
- одинаковое содержимое помечается синхронизированным без лишнего коммита;
- одинаковая дата при разном содержимом и незафиксированные внешние изменения останавливают операцию без перезаписи.
